...
function RentalCreatePage() {
  ...
  const formik = useFormik<RentalInterface>({
    initialValues: {
      rent_start_date: new Date(new Date().toDateString()),
      rent_end_date: new Date(new Date().toDateString()),
      total_cost: 0,
    ...
    >
      <Box rounded="md">
        ...
        <FormWrapper onSubmit={formik.handleSubmit}>
          <FormControl id="rent_start_date" mb="4">
            <FormLabel fontSize="1rem" fontWeight={600}>
              Rent Start Date
            </FormLabel>
            <DatePicker
              selected={formik.values?.rent_start_date ? new Date(formik.values?.rent_start_date) : null}
              onChange={(value: Date) => formik.setFieldValue('rent_start_date', value)}
            />
          </FormControl>
          <FormControl id="rent_end_date" mb="4">
            <FormLabel fontSize="1rem" fontWeight={600}>
              Rent End Date
            </FormLabel>
            <DatePicker
              selected={formik.values?.rent_end_date ? new Date(formik.values?.rent_end_date) : null}
              onChange={(value: Date) => formik.setFieldValue('rent_end_date', value)}
            />
          </FormControl>
